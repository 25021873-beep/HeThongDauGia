...
class AutoBidServiceTest {
    ...
    void registerAutoBidHappyCaseSavesConfig() {
        StubAuctionDAO auctionDAO = new StubAuctionDAO(runningAuction(new BigDecimal("1000")));
        StubUserDAO userDAO = new StubUserDAO();
        RecordingAutoBidDAO autoBidDAO = new RecordingAutoBidDAO();
        userDAO.put(bidder(2, new BigDecimal("5000")));

        AutoBidService service = new AutoBidService(autoBidDAO, AuctionService.getInstance(),
                auctionDAO, userDAO, new StubBidTransactionDAO(null));
    ...
    void triggerAutoBidSkipsTheBidderWhoTriggeredCurrentBid() {
        ...
        assertTrue(autoBidDAO.deactivatedKeys.isEmpty(), "Nguoi vua bid khong duoc auto-bid lai chinh minh");
    }
